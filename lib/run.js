...
worker.defineGetter(worker, 'siteRootPath', function(){
	return (worker.serverConf&&worker.serverConf.path||null);
});
worker.defineSetter(worker, 'siteRootPath', function(value){
	worker.serverConf.path = value;
});
...
worker._start = function(siteConfig){
	//站点配置
	worker.siteConfig = siteConfig ;
	let wsMain = worker.siteConfig&&worker.siteConfig.wsMain||null;
	//ws
	if (wsMain) {
		wsMain = path.join(worker.siteRootPath, wsMain);
		require(wsMain)(worker);
	}
	//http
	require('./http')(worker);

	if(worker.siteConfig.defaultListen!==undefined){
		worker.serverConf.defaultListen = worker.siteConfig.defaultListen ;
	}
	worker.serverConf.listen = worker.siteConfig.listen || worker.serverConf.listen || [];
	worker.serverConf.cpuLen = worker.siteConfig.cpuLen || worker.serverConf.cpuLen || 1;

	worker.callMaster('updateServerConf', worker.serverConf, (state, message)=>{
		if (state) {
			console.log('updateServerConf 更新了');
		}else{
			console.log('updateServerConf 更新失败');
		}
	...
	worker.server.ConnectionHttp = class ConnectionHttp extends domain.Domain{
		...
		__init(){
			...
			//用户访问协议 - http | https
			this.request.router.protocol = this.request.socket.info.protocol = (this.request.type==='ssl'?'https:':'http:');
...
module.exports = {
	start:function(siteConfig){
		siteConfig = siteConfig || Object.create(null);
		if (initEnd===true) {
			if (siteConfig) {
