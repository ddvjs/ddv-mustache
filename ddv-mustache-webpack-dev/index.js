...
function ddvMustacheWebpackDevInstall () {
  ...
  return new Promise((resolve, reject) => {
    var ls = spawn('npm', ['--save-dev', 'install', 'ddv-mustache-webpack-dev'], {
      cwd: process.cwd(),
    ...
    ls.on('close', (code) => {
      if (code === 0) {
        ...
        reject(new Error('Please allow the command to install dependent packages: npm -D i ddv-mustache-webpack-dev'))
      }
    })
  })
}
function checkHasDdvMustacheWebpackDev () {
  ...
  return new Promise((resolve, reject) => {
    var ls = spawn('npm', ['ls', 'ddv-mustache-webpack-dev'], {
      cwd: process.cwd(),
      stdio: ['pipe', 'inherit', 'inherit']
    })
    ls.on('close', (code) => {
